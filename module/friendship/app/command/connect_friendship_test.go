...
import (
	...
	mockFriendshipRepo "github.com/phantranhieunhan/s3-assignment/mock/friendship/repository"
...
func TestFriendship_ConnectFriendship(t *testing.T) {
	t.Parallel()
	mockRepo := new(mockFriendshipRepo.MockFriendshipRepository)
	mockTransaction := new(mockFriendshipRepo.MockTransaction)
	h := NewConnectFriendshipHandler(mockRepo, mockTransaction)
	...
	tcs := []struct {
	...
	}{
		{
			...
			setup: func(ctx context.Context) {
				mockRepo.On("GetFriendshipByUserID", ctx, friends[0], friends[1]).Return(domain.Friendship{}, domain.ErrRecordNotFound).Once()
				...
				mockRepo.On("Create", ctx, domain.Friendship{UserID: friends[0], FriendID: friends[1], Status: domain.FriendshipStatusFriended}).Return(friendshipId, nil).Once()
			},
			err: nil,
		},
		{
			...
			setup: func(ctx context.Context) {
				mockRepo.On("GetFriendshipByUserID", ctx, friends[0], friends[1]).Return(domain.Friendship{
					Base: domain.Base{
						Id:        friendshipId,
						CreatedAt: now,
						UpdatedAt: now,
				...
				mockRepo.On("UpdateStatus", ctx, friendshipId, domain.FriendshipStatusFriended).Return(nil).Once()
			},
			err: nil,
		},
		{
		...
		},
		{
			...
			setup: func(ctx context.Context) {
				mockRepo.On("GetFriendshipByUserID", ctx, friends[0], friends[1]).Return(domain.Friendship{}, errDB).Once()
				mockTransaction.On("WithinTransaction", ctx, mock.Anything).Run(func(args mock.Arguments) {
					f := args[1].(func(ctx context.Context) error)
					err := f(ctx).(*common.AppError)
		...
		},
		{
			...
			setup: func(ctx context.Context) {
				...
				mockRepo.On("Create", ctx, domain.Friendship{UserID: friends[0], FriendID: friends[1], Status: domain.FriendshipStatusFriended}).Return("", common.ErrCannotCreateEntity(friendship.DomainName(), errDB)).Once()
			},
		...
		},
		{
			...
			setup: func(ctx context.Context) {
				...
				mockRepo.On("UpdateStatus", ctx, friendshipId, domain.FriendshipStatusFriended).Return(common.ErrCannotUpdateEntity(friendship.DomainName(), errDB)).Once()
			},
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			id, err := h.Create(ctx, friendship)
			assert.Equal(t, err, tc.err)
			if tc.err == nil {
				assert.Equal(t, friendshipId, id)
			}
			mock.AssertExpectationsForObjects(t, mockRepo, mockTransaction)
		})
	}
}
